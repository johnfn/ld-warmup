	...
	public updateText(): void {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			for (let j = 0; j < outputTextData[i].length; j++) {
				...
				const textStyle = new PIXI.TextStyle(sty);
		...
		for (let i = 0; i < outputTextData.length; i++) {
			...
			for (let j = 0; j < line.length; j++) {
				...
				if (style.letterSpacing === 0) {
					...
					this.context.font = new PIXI.TextStyle(line[j].style).toFontString();
		...
		drawingData.forEach(({ style, text, x, y }) => {
			...
			this.context.font = new PIXI.TextStyle(style).toFontString();
